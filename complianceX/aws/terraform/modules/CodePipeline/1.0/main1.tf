...
resource "aws_codedeploy_deployment_group" "deployment_group" {
  ...
  dynamic "ec2_tag_filter" {
    for_each = var.DEPLOYMENT_GROUP[count.index].ec2_tag_filter
    content {
  ...
  dynamic "alarm_configuration" {
    for_each = var.DEPLOYMENT_GROUP[count.index].alarm_configuration

    content {
      alarms  = lookup(alarm_configuration.value, "alarms", [])
      enabled = lookup(alarm_configuration.value, "enabled", true)
    }
  }
      ...
      dynamic "action" {
        ...
        content {
          ...
          configuration = {
            ...
            "FileName"            = action.value.configuration["FileName"]
          }
        }
      }
    }
